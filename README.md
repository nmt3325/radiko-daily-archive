...
番組ごとの取得結果は `manifest.json` の `status` に記録されます。

| status | 意味 | Jobの扱い |
|---|---|---|
| `downloaded` | 取得成功 | 成功 |
| `unavailable` | radiko側がタイムフリーで配信していない（権利上の制限、配信停止、聴取専用など）。再試行しても取得できないため即座にスキップ | 成功のまま |
| `failed` | 通信・API障害などで再試行しても取得できなかった | 失敗 |

`unavailable` はradiko側の仕様であってアーカイブ側の異常ではないため、Jobやワークフローを失敗させません。局内の全番組が取得できなかった場合のみJobが失敗します。成功済みの番組と局アーカイブはDraft Releaseに保存されます。
